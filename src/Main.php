...
class Main {
	private $plugin_url = '';
	private $assets_enqueued = 0;

	public function __construct( $plugin_url ) {
		$this->plugin_url = $plugin_url;

		add_action( 'wp_ajax_load_attendee_data_editor_form',[ $this, 'editor_content' ] );
		add_action( 'wp_ajax_save_attendee_data_editor_form', [ $this, 'save_fields' ] );
		add_action( 'admin_init', [ $this, 'add_assets' ] );
		add_filter( 'tribe_events_tickets_attendees_table_column', [ $this, 'add_edit_link' ], 20, 3 );
	}
	...
	protected function assets() {
		wp_enqueue_style(
			...
			$this->plugin_url . 'src/resources/css/attendee-data-editor.css'
		);

		wp_enqueue_script(
			'attendee-data-editor-script',
			$this->plugin_url . 'src/resources/js/attendee-data-editor.js',
			[ 'jquery' ],
			false,
			true
		);
	...
	public function add_edit_link( $value, $item, $column ) {
		...
		$label = esc_html__( 'Edit details', 'tribe-ext-attendee-data-editor' );
		$link  = " 
			<div> <a
	...
	public function editor_content() {
		...
		ob_start();
		Tickets_View::instance()->output_attendee_meta( $attendee_data );
	...
	protected function editor_content_checks() {
		return (
			...
			&& wp_verify_nonce( $_POST['check'], 'attendee-data-editor' )
		);
	}
	...
	protected function get_attendee_array() {
		...
		foreach ( Tickets_View::instance()->get_event_attendees_by_order( $event_id ) as $order ) {
